...
resource "null_resource" "haproxy_install" {
  ...
  provisioner "local-exec" {
    command = "cd ansible/haproxy && ansible-playbook -i ../../config/hosts.ini -b -u ${var.vm_ssh_user} -e \"ansible_ssh_pass=$VM_PASSWORD ansible_become_pass=$VM_PRIVILEGE_PASSWORD\" ${local.extra_args[var.vm_distro]} -v haproxy.yml"
...
resource "null_resource" "kubespray_create" {
  ...
  provisioner "local-exec" {
    command = "cd ansible/kubespray && ansible-playbook -i ../../config/hosts.ini -b -u ${var.vm_ssh_user} -e \"ansible_ssh_pass=$VM_PASSWORD ansible_become_pass=$VM_PRIVILEGE_PASSWORD kube_version=${var.k8s_version}\" ${local.extra_args[var.vm_distro]} -v cluster.yml"
...
resource "null_resource" "kubespray_add" {
  ...
  provisioner "local-exec" {
    command = "cd ansible/kubespray && ansible-playbook -i ../../config/hosts.ini -b -u ${var.vm_ssh_user} -e \"ansible_ssh_pass=$VM_PASSWORD ansible_become_pass=$VM_PRIVILEGE_PASSWORD kube_version=${var.k8s_version}\" ${local.extra_args[var.vm_distro]} -v scale.yml"
...
resource "null_resource" "kubespray_upgrade" {
  ...
  provisioner "local-exec" {
    command = "cd ansible/kubespray && ansible-playbook -i ../../config/hosts.ini -b -u ${var.vm_ssh_user} -e \"ansible_ssh_pass=$VM_PASSWORD ansible_become_pass=$VM_PRIVILEGE_PASSWORD kube_version=${var.k8s_version}\" ${local.extra_args[var.vm_distro]} -v upgrade-cluster.yml"
...
resource "proxmox_vm_qemu" "worker" {
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "cd ansible/kubespray && ansible-playbook -i ../../config/hosts.ini -b -u ${self.ssh_user} -e \"ansible_ssh_pass=$VM_PASSWORD ansible_become_pass=$VM_PRIVILEGE_PASSWORD node=$VM_NAME delete_nodes_confirmation=yes\" -v remove-node.yml"
